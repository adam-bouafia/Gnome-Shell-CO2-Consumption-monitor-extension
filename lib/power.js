 ...
 * Returns an array of { name, energyPath, maxRangeUj } objects.
 * Caches result after first call.
 */
export function detectRaplDomains() {
    if (_raplDomains !== null) return _raplDomains;
    _raplDomains = [];
    const basePath = '/sys/class/powercap';
    try {
        const dir = Gio.File.new_for_path(basePath);
        if (!dir.query_exists(null)) return _raplDomains;
        ...
        while ((info = enumerator.next_file(null)) !== null) {
            ...
            const maxPath = `${basePath}/${name}/max_energy_range_uj`;
            // Check readability (requires kernel config or udev rule)
            const energyText = readFile(energyPath);
            if (energyText === null) continue;
            const maxText = readFile(maxPath);
            const maxRangeUj = maxText ? parseInt(maxText.trim(), 10) : 0;
            _raplDomains.push({ name, energyPath, maxRangeUj });
        }
...
export function raplAvailable() {
    return detectRaplDomains().length > 0;
}
...
export function readRaplSnapshot() {
    const domains = detectRaplDomains();
    if (domains.length === 0) return null;
    let totalUj = 0;
    for (const d of domains) {
        const text = readFile(d.energyPath);
 ...
 */
export function getHeuristicCpuWatts(cpuPercent, profile = 'desktop') {
...
export function getInstalledRamGB() {
 ...
 */
export function getRamPowerWatts() {
    return getInstalledRamGB() * RAM_WATTS_PER_GB;
}
 ...
 * @returns {{ watts: number, source: 'rapl'|'heuristic' }}
 */
export function getTotalPowerWatts({ prevRapl, currRapl, cpuPercent, profile }) {
    const ramW = getRamPowerWatts();
    ...
    const cpuW = getHeuristicCpuWatts(cpuPercent, profile);
