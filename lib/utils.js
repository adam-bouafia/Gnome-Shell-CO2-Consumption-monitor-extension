...
/** Promise-based sleep using the GLib main loop; cancellable on disable. */
...
/** Read a file synchronously, returning its text content or null. */
export function readFile(path) {
    try {
        const file = Gio.File.new_for_path(path);
        const [ok, contents] = file.load_contents(null);
        if (ok) return decodeUtf8(contents);
    } catch (_) {}
    return null;
}
