...
async function detectCountryCode() {
    try {
        const text = await httpGet('https://ipapi.co/json/');
        if (!text) return null;
        const obj = JSON.parse(text);
...
export async function getCachedCountryCode() {
 ...
 */
export function loadCountryIntensityMap(basePath) {
 ...
 */
export async function autoCountryIntensity(basePath) {
    const code = await getCachedCountryCode();
    if (!code) return null;
    const map = loadCountryIntensityMap(basePath);
...
export async function resolveIntensity(opts) {
    const {
        ...
        fixedIntensity, basePath, cache, cacheTtl,
    } = opts;
    ...
    if (provider === 'electricitymaps') {
        ...
        const val = await fetchElectricityMapsIntensity(apiKey, effectiveZone);
        if (typeof val === 'number' && val > 0) {
    ...
    if (provider === 'auto-country') {
        const result = await autoCountryIntensity(basePath);
        if (result) {
