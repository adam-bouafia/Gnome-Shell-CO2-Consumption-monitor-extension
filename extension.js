...
function getSettings() {
    ...
    try {
        ...
        try { return new Gio.Settings({ schema_id: 'org.gnome.shell.extensions.co2consumption' }); } catch (_) { throw e; }
    }
}
...
// Resolve extension directory path once
function getExtensionDir() {
    try {
        const thisFile = Gio.File.new_for_uri(import.meta.url);
        return thisFile.get_parent().get_path();
    } catch (_) {
        return null;
    }
}
...
function bytesToString(bytes) {
    // Prefer modern API: GLib.Bytes.toArray (GJS >= 1.76)
    try {
        if (bytes && typeof bytes.toArray === 'function') {
            return _decodeUtf8(bytes.toArray());
        }
    } catch (_) {}
    // Fallback to legacy imports.byteArray if available
    try {
        const legacy = globalThis.imports?.byteArray;
        if (legacy?.fromGBytes) {
            const arr = legacy.fromGBytes(bytes);
            if (legacy.toString)
                return legacy.toString(arr);
            return _decodeUtf8(arr);
        }
    } catch (_) {}
    return null;
}
...
async function fetchElectricityMapsIntensity(apiKey, zone) {
    ...
    try {
        ...
        log(`CO2 Monitor: ElectricityMaps fetch error: ${e}`);
    }
    return null;
}
...
async function _getSoup() {
    try {
        ...
        try {
            ...
            log('CO2 Monitor: libsoup not available; HTTP disabled');
            return null;
        }
    }
}
...
async function detectCountryCode() {
    ...
    try {
        ...
        log(`CO2 Monitor: geolocation error: ${e}`);
    }
    return null;
}
...
function loadCountryIntensityMap() {
    ...
    try {
        ...
        log(`CO2 Monitor: failed to load bundled country intensities: ${e}`);
        _bundledCountryMap = COUNTRY_INTENSITY_FALLBACK;
    }
    return _bundledCountryMap;
}
...
async function fetchOWIDCountryIntensity() {
    ...
    try {
        ...
        log(`CO2 Monitor: OWID fetch error: ${e}`);
    }
    return null;
}
...
async function sampleProcessShares(sampleMs = 250) {
    ...
    await new Promise(r => GLib.timeout_add(GLib.PRIORITY_DEFAULT, sampleMs, () => { r(); return GLib.SOURCE_REMOVE; }));
...
class CO2Indicator extends PanelMenu.Button {
    ...
    this._settings = getSettings();
        ...
        log('CO2 Consumption Monitor: Initializing indicator');
    ...
    this._rolling = []; // keep last N totals for smoothing
        ...
        // Immediate UI reactions for specific settings
    ...
    const headerControls = new St.BoxLayout({ vertical: false });
        ...
        applyPopupOpacity();
        this._settings.connect('changed::popup-opacity', () => applyPopupOpacity());
    ...
    const settingsIcon = new St.Icon({ icon_name: 'emblem-system-symbolic', style_class: 'system-status-icon', icon_size: 20 });
        ...
        settingsBtn.connect('clicked', () => {
            try {
                GLib.spawn_command_line_async('gnome-extensions prefs co2consumption@gmail.com');
            } catch (e) {
                log(`CO2 Monitor: open prefs failed: ${e}`);
            }
        });
    ...
    const clearIcon = new St.Icon({ icon_name: 'user-trash-symbolic', style_class: 'system-status-icon', icon_size: 20 });
        ...
        clearBtn.connect('clicked', () => {
            try {
                ...
                log(`CO2 Monitor: clear totals failed: ${e}`);
            }
        });
        // We'll add this button to the controls container later (bottom-right)
        ...
        try {
            ...
            if (!safe && this._settings.get_boolean('per-software-monitoring')) {
                try {
                    ...
                    log(`CO2 Monitor: per-process sampling error: ${e}`);
                }
            }
            ...
            log(`CO2 Monitor: estimation error: ${e}`);
    ...
    _exportOverallSoftwareTotalsCSV(viewAll = false) {
        try {
            ...
            const toBytes = (s) => (globalThis.imports?.byteArray?.fromString) ? globalThis.imports.byteArray.fromString(s) : new TextEncoder().encode(s);
            stream.write_all(toBytes(header + body), null);
            stream.close(null);
        } catch (e) {
            log(`CO2 Monitor: export overall software CSV failed: ${e}`);
        }
    }
    ...
    _importOverallSoftwareTotalsCSV() {
        try {
            ...
            log(`CO2 Monitor: import overall software CSV failed: ${e}`);
        }
    }
    ...
    _exportDailyHistoryCSV() {
        try {
            ...
            log(`CO2 Monitor: export daily history failed: ${e}`);
        }
    }
    ...
    _exportTotalsToCSV() {
        try {
            ...
            log(`CO2 Monitor: export CSV failed: ${e}`);
        }
    }
    ...
    _setupOrRefreshPeriodicExport() {
        ...
        try {
            ...
            this._periodicExportId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, sec, () => {
                try { this._exportDailyHistoryCSV(); } catch (e) { log(`CO2 Monitor: periodic export failed: ${e}`); }
                return GLib.SOURCE_CONTINUE;
            });
        } catch (e) {
            log(`CO2 Monitor: periodic schedule error: ${e}`);
        }
    }
...
export default class Extension {
    ...
    enable() {
        log('CO2 Consumption Monitor: Enabling extension');
        try {
            this._indicator = new CO2Indicator();
            Main.panel.addToStatusArea('co2-monitor', this._indicator);
        } catch (e) {
            log(`CO2 Consumption Monitor: enable() error: ${e}`);
            if (this._indicator) {
    ...
    disable() {
        log('CO2 Consumption Monitor: Disabling extension');
        if (this._indicator) {
...
// Legacy loader compatibility for GNOME 43–44
let _legacy = { indicator: null };
export function init() {
    // nothing else to init eagerly
}
export function enable() {
    if (_legacy.indicator) return;
    try {
        _legacy.indicator = new CO2Indicator();
        Main.panel.addToStatusArea('co2-monitor', _legacy.indicator);
    } catch (e) {
        log(`CO2 Consumption Monitor: legacy enable() error: ${e}`);
        if (_legacy.indicator) {
            try { _legacy.indicator.destroy(); } catch (_) {}
            _legacy.indicator = null;
        }
        throw e;
    }
}
