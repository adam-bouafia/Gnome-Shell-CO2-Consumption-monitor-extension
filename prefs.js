...
class CO2ConsumptionPreferences extends Adw.PreferencesPage {
    _init(settings) {
        ...
        // settings provided by caller (GNOME 45+)
        ...
        linkRow1.connect('activated', () => {
            try { Gio.AppInfo.launch_default_for_uri('https://adam-bouafia.github.io/', null); } catch (e) { console.warn(`CO2 Prefs: open URL failed: ${e}`); }
        });
        ...
        linkRow2.connect('activated', () => {
            try { Gio.AppInfo.launch_default_for_uri('https://www.linkedin.com/in/adam-bouafia-b597ab86/', null); } catch (e) { console.warn(`CO2 Prefs: open URL failed: ${e}`); }
        });
    ...
    _importHistoryFromPath(path) {
        try {
            ...
            console.warn(`CO2 Prefs: import error: ${e}`);
        }
    }
});
// GNOME 45+: use ExtensionPreferences to obtain settings
export default class Preferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        const settings = this.getSettings();
        const page = new CO2ConsumptionPreferences(settings);
        window.add(page);
    }
}
